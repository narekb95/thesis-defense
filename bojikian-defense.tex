...
\begin{frame}[t]{Reduction - $T$-Path Packing to Matching}
	...
	\begin{minipage}[t][.4\textheight][t]{\linewidth}
		\centering
		\uncover<2->{\includegraphics[width=.5\textwidth]{figures/reductions/graph-first.eps}}
	\end{minipage}
	\hrule
...
\begin{itemize}
	...
	\uncover<6->{\item If maximum matching can be solved efficiently on $G$, it can be solved efficiently on $f(G)$.}
	\end{itemize}
\end{frame}
...
\begin{itemize}
	...
	\uncover<2->{\item Odd $T$-Path Packing.\\
		...
		\begin{itemize}
			...
			\uncover<4->{\item[--] For $m := |E(G)|$, $l, k \in \mathbb{N}$,  if $d_{\mathcal{R}[\ell]}(G) \leq k$,\\
...
\begin{frame}[t]{Methods and results - Simplify, solve and augment}
	...
	\begin{align*}
		...
		&= O(\tm_{A}(G \setminus S) + |S|(n+m)),
	\end{align*}
	where $n$ is the number of the vertices and $m$ is the number edges in $G$.
}
\end{frame}
